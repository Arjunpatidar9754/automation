...
describe('Child profile new design',() => {
    ...
    //                                         cy.wait(2000)
        ...
        //Downloading the uploaded file
            cy.get('#achivementsViewCard').find(':nth-child(3) >.edit-icon.float-start').click({force:true})
                cy.wait(2000)  //image is downoaded

            cy.get('#achivementsViewCard').find('.edit-icon.float-start').click({force:true})
                cy.wait(2000) //pdf is downoaded

        //Removing the uploaded file from the card
            cy.get('#achivementsViewCard').find('.edit-icon float-start ms-3').click({force:true})
                    cy.wait(2000)
                    cy.get('.react-confirm-alert-button-group').contains('No').click()
                        cy.wait(1000)
                            cy.get('#addOptionView_14').find('.edit-icon float-start ms-3').click({force:true})
                                cy.wait(2000)
