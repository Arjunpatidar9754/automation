...
describe('Child profile new design',() => {
    ...
    it('Child intro',function()
    {
        ...
        cy.get('input[name=email]').type('meera@mailinator.com')
        cy.get('input[name=password]').type('MlFWrOTd')
        cy.get('.btn').click()
            cy.wait(3000)
        cy.url().should('include','child')
            cy.wait(1000)
    //Checking the cancel button section
        cy.get('#addOptionView_10').find('.btn.btn-border').contains('Add now').click()
